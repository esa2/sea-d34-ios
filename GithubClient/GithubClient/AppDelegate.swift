...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    if let token = NSUserDefaults.standardUserDefaults().objectForKey("githubToken") as? String {
      ...
      if let
        ...
        storyboard = rootViewController.storyboard {
          let mainMenuViewController = storyboard.instantiateViewControllerWithIdentifier("MainMenuView") as! UINavigationController
  ...
  func application(application: UIApplication, openURL url: NSURL, sourceApplication: String?, annotation: AnyObject?) -> Bool {
    ...
    // check if safari etc, error checking
    self.oauthService.handleRedirect(url) // fired when coming back from github
    return true
  }
