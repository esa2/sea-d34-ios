...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
  ...
  if (!self.isForTesting) {
    options = @{NSMigratePersistentStoresAutomaticallyOption : @true, NSInferMappingModelAutomaticallyOption : @true,NSPersistentStoreUbiquitousContentNameKey : @"HoteliCloud", NSPersistentStoreUbiquitousContentURLKey : [self cloudDirectory]};
  }
  ...
  if (results == 0) {
    ...
    if (!jsonError) {
      ...
      for (NSDictionary *hotelDictionary in jsonArray) {
        ...
        hotel.swimmingPool = hotelDictionary[@"pool"];
        hotel.roomService = hotelDictionary[@"roomservice"];
